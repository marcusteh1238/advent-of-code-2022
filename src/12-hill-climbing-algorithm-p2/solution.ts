...
class BFSForest {
    ...
    getPath() {
        const visited: Tree[] = [];
        // start from end
        let queue: Tree[] = [this.end];
        while (queue.length > 0) {
            const tree = queue.shift();
            if (visited.some(t => t.is(tree))) continue;
            visited.push(tree);
            // stop where there is a tree of 0 height
            if (tree.height === 0) {
                this.start = tree;
                break;
            };
    ...
    getUnvisitedNeighbours(tree: Tree, visited: Tree[]) {
        ...
        // change here: we want to go down instead now
        return unvisited.filter(t => tree.height - t.height <= 1);
    }
